...
def F1Score(hist):
    ...
    recall = np.diag(hist) / hist.sum(axis=0)
    f1score = 2 * per_class_PA(hist) * recall / (per_class_PA(hist) + recall)
    return f1score
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes ):  
    ...
    for ind in range(len(gt_imgs)): 
        ...
        if ind > 0 and ind % 10 == 0:  
            print('{:d} / {:d}: mIou-{:0.2f}; mPA-{:0.2f}; OA-{:0.2f}; FWIoU-{:0.2f}; F1Score-{:0.2f};'.format(ind, len(gt_imgs),
                                                    100 * np.mean(per_class_iu(hist)),
                                                    100 * np.mean(per_class_PA(hist)),
                                                    100 * np.mean(per_class_OA(hist)),
                                                    100 * np.mean(per_class_FWIoU(hist)),
                                                    100 * np.mean(F1Score(hist))))
    # 计算所有验证集图片的逐类别mIoU值
    mIoUs   = per_class_iu(hist)
    mPA     = per_class_PA(hist)
    OA      = per_class_OA(hist)
    FWIoU   = per_class_FWIoU(hist)
    F1score = F1Score(hist)
    ...
    print('===> mIoU: ' + str(round(np.nanmean(mIoUs) * 100, 2)) + '; mPA: ' + str(round(np.nanmean(mPA) * 100, 2)) +
          "; OA:" + str(round(np.nanmean(OA)*100, 2)) + "; FWIoU:" + str(round(np.nanmean(FWIoU)*100, 2)) + "; F1score:"
          + str(round(np.nanmean(F1score)*100, 2)))
    return mIoUs


if __name__ == "__main__":
    gt_dir = "DataSet/DataSet/SegmentationClass"
    pred_dir = "miou_pr_dir"
    png_name_list = open(r"DataSet/DataSet/ImageSets/Segmentation/val.txt", 'r').read().splitlines()

    num_classes = 2
    name_classes = ["background", "lodging"]
