...
from nets.unet import ResNet50UnetPlus
...
def fit_one_epoch(net, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda, aux_branch):
    ...
    file_handle2.write(totalBig_loss + '\n')
    file_handle2.close()
    # 记录测试集的loss
    file_handle2 = open('val_loss.csv', mode='a+')
    file_handle2.write(val_loss1232 + '\n')
    file_handle2.close()
    # 记录验证集的f1-score
...
if __name__ == "__main__":
    ...
    model = ResNet50UnetPlus(num_class=NUM_CLASSES).train()
